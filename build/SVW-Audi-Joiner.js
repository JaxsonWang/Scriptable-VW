...
class Widget extends DataRender {
  ...
  constructor(arg) {
    super(arg);
    this.name = '上汽奥迪挂件';
    this.desc = '上汽奥迪车辆桌面组件展示';

    this.appId = 'com.svw.sc.audi';
    this.appName = 'SVW-Audi';
    this.appVersion = '1.0';
    this.brand = 'Audi';

    this.myCarPhotoUrl = `${this.getStaticUrl()}/build/assets/images/svw_audi_default.png`;
  ...
  async handleLoginRequest(debug = false) {
    const options = {
      url: 'https://audi-api.mos.csvw.com/mos/security/api/v1/app/actions/pwdlogin',
      method: 'POST',
      headers: {
        ...this.requestHeader(),
        ...{
      ...
      body: JSON.stringify({
        pwd: this.settings['password'].trim(),
        mobile: this.settings['username'].trim(),
        picContent: '',
        picTicket: '',
        deviceId: this.getDid(),
        scope: 'openid',
        brand: 'audi',
        deviceType: 'ios'
      })
  ...
  async getTokenRequest(type, debug = false) {
    ...
    case 'refreshAuthToken':
      requestParams = `grant_type=${encodeURIComponent('id_token')}&token=${encodeURIComponent(this.settings['userIDToken'])}&scope=${encodeURIComponent('t2_saudi:fal')}`;
      break
    case 'authAccessToken':
      requestParams = `grant_type=${encodeURIComponent('refresh_token')}&token=${encodeURIComponent(this.settings['refreshAuthToken'])}&scope=${encodeURIComponent('t2_saudi:fal')}&vin=${this.settings['carVIN']}`;
      break
    }
  ...
  async getUserMineRequest(debug = false) {
    const options = {
      url: 'https://audi-api.mos.csvw.com/mos/security/api/v1/auth/token/relationship',
      method: 'GET',
      headers: {
        ...{
    ...
    try {
      ...
      // 判断接口状态
      if (response.code === '000000') {
        const { vehicle: { serial, model, vin } } = response.data;
        this.settings['seriesName'] = serial;
        this.settings['carModelName'] = model;
        this.settings['carVIN'] = vin;
        await this.saveSettings(false);
        console.log('获取用户基本信息成功并将存储本地');
        if (debug) {
          console.log('获取个人信息：');
          console.log('车系名称：' + serial);
          console.log('车型名称：' + model);
          console.log('车架号码：' + vin);
        }
        // 准备交换验证密钥数据
        await this.getTokenRequest('authAccessToken', debug);
      } else {
        console.error('获取个人信息失败，请登出重置后再进行小组件登录！' + response.description);
        await this.notify('个人信息获取失败', '获取个人信息失败，请登出重置后再进行小组件登录！' + response.description);
      }
    } catch (error) {
      console.error(error);
    }
  ...
  async actionAccountLogin() {
    const message = `
      Joiner 小组件需要使用到您的上汽奥迪应用的账号，首次登录请配置账号、密码进行令牌获取\n\r
      Joiner 小组件不会收集您的个人账户信息，所有账号信息将存在 iCloud 或者 iPhone 上但也请您妥善保管自己的账号\n\r
    ...
    if (present !== -1) {
      ...
      alert.message = '使用上汽奥迪账号登录进行展示数据';
      alert.addTextField('上汽奥迪账号', this.settings['username']);
      alert.addSecureTextField('上汽奥迪密码', this.settings['password']);
      alert.addAction('确定');
      alert.addCancelAction('取消');
   ...
   */
  getDid() {
    return `AUDI_APP_iPhone_${this.settings['clientID'].replace(/-/g, '')}_9_4.3.4`
  }
